...
export interface SessionManager {
  ...
  spawnAndSend(
    ...
    sessionMode: SessionMode,
    text: string,
    contextChips?: Array<{ filePath: string; fileName: string; isDirectory?: boolean; range?: { startLine: number; endLine: number } }>,
  ): Promise<void>;
...
export function createSessionManager(config: SessionManagerConfig): SessionManager {
  ...
  function switchToInstance(instanceKey: string): void {
    ...
    if (!skipClient) {
      config.onCoreSession?.(target)?.catch((e: unknown) =>
        adapter.log(`[SessionManager] Failed to initialize core session: ${e}`),
      );
    }
  ...
  async function spawnAgent(agentType: string, sessionMode: SessionMode = "single_agent"): Promise<void> {
    ...
    try {
      ...
      config.onCoreSession?.(instance)?.catch((e: unknown) =>
        adapter.log(`[SessionManager] ensureCoreSession failed (non-fatal): ${e}`),
      );
    } catch (error) {
  ...
  function closeInstance(instanceKey: string): void {
    ...
    if (instance.client) {
      try { instance.client.dispose(); } catch {}
    }
  ...
  async function sendMessageImpl(
    ...
    try {
      ...
      config.onCoreSession?.(inst)?.catch((e: unknown) =>
        adapter.log(`[SessionManager] ensureCoreSession failed (non-fatal): ${e}`),
      );
  ...
  return {
    ...
    sendInstanceList,
    sendSessionMetadata,
    setMessageInterceptor: (fn) => { messageInterceptor = fn; },
    dispose,
  };
}
