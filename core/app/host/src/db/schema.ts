...
const TABLES: string[] = [
  ...
  // Optimized system prompts generated by scripts/optimize-prompts.ts.
  // Written by the optimizer script, read per-run by the orchestrator.
  // Falls back to default static prompts when absent.
  `CREATE TABLE IF NOT EXISTS optimized_prompts (
    target_step   TEXT PRIMARY KEY,  -- 'taskDecompose' | 'agentSelect' | 'promptBuild' | 'progressEval'
    system_prompt TEXT NOT NULL,
    generated_at  INTEGER NOT NULL,
    repo_profile  TEXT NOT NULL,     -- JSON snapshot of profile used for generation
    model         TEXT NOT NULL      -- model string used for generation
  )`,
];
  ...
  for (let v = fromVersion + 1; v <= toVersion; v++) {
    switch (v) {
      case 1:
        ...
        break;
      case 2:
        // Add optimized_prompts table (written by scripts/optimize-prompts.ts).
        _db.exec(
          `CREATE TABLE IF NOT EXISTS optimized_prompts (
            target_step   TEXT PRIMARY KEY,
            system_prompt TEXT NOT NULL,
            generated_at  INTEGER NOT NULL,
