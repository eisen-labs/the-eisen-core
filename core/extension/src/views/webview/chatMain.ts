...
export function ansiToHtml(text: string): string {
  ...
  while ((match = ANSI_RE.exec(text)) !== null) {
    ...
    for (const code of match[1].split(";").map((c) => parseInt(c, 10) || 0)) {
      if (code === 0) classes = [];
      else if (ANSI_STYLE[code]) {
        if (!classes.includes(ANSI_STYLE[code])) classes.push(ANSI_STYLE[code]);
      } else if (ANSI_FG[code]) {
        classes = classes.filter(
          (c) =>
            !c.startsWith("ansi-") ||
            c.startsWith("ansi-bg-") ||
            c.startsWith("ansi-bold") ||
            c.startsWith("ansi-dim") ||
            c.startsWith("ansi-italic") ||
            c.startsWith("ansi-underline"),
        );
        classes.push(ANSI_FG[code]);
      } else if (ANSI_BG[code]) {
        classes = classes.filter((c) => !c.startsWith("ansi-bg-"));
        classes.push(ANSI_BG[code]);
      }
    }
...
export function renderDiff(
  ...
  html += "</pre>";
  if (truncated)
    html += `<div class="diff-truncated">... (truncated, showing first 500 of ${lines.length} lines)</div>`;
  html += "</div>";
  return html;
}
...
export function getToolsHtml(tools: Record<string, Tool>, expandedToolId?: string | null): string {
  ...
  const items = ids
    .map((id) => {
      ...
      if (tool.output) {
        ...
        details +=
          ...
          (hasAnsi ? " terminal" : "") +
          '">' +
          (hasAnsi ? ansiToHtml(trunc) : escapeHtml(trunc)) +
          "</pre>";
      }
    ...
    .join("");
  return (
    '<details class="tool-details" open><summary>' +
    ids.length +
    " tool" +
    (ids.length > 1 ? "s" : "") +
    '</summary><ul class="tool-list">' +
    items +
    "</ul></details>"
  );
}
...
class WebviewController {
  ...
  private restore(): void {
    ...
    if (s.instanceHtmlCache)
      for (const [id, html] of Object.entries(s.instanceHtmlCache)) this.instanceHtmlCache.set(id, html);
    if (s.instanceUiStates)
      for (const [id, st] of Object.entries(s.instanceUiStates)) this.instanceUiStates.set(id, st);
    if (this.currentInstanceKey) {
      ...
      if (ui) {
        ...
        this.renderChips();
        if (ui.usageMeterText) {
          this.el.usageMeterEl.textContent = ui.usageMeterText;
  ...
  private save(): void {
    ...
    this.instanceHtmlCache.forEach((v, k) => {
      html[k] = v;
    });
    const ui: Record<string, PerInstanceUiState> = {};
    this.instanceUiStates.forEach((v, k) => {
      ui[k] = v;
    });
    if (this.currentInstanceKey) {
      ui[this.currentInstanceKey] = {
    ...
    this.vscode.setState<WebviewState>({
      ...
      inputValue: this.el.inputEl.value,
      currentInstanceKey: this.currentInstanceKey,
      instanceHtmlCache: html,
      instanceUiStates: ui,
    });
  }
  ...
  private bind(win: Window): void {
    ...
    inputEl.addEventListener("keydown", (e) => {
      if (this.filePickerVisible && this.fileResults.length > 0) {
        if (e.key === "ArrowDown") {
          e.preventDefault();
          this.selectedFileIdx = Math.min(this.selectedFileIdx + 1, this.fileResults.length - 1);
          this.renderFileList();
          return;
        }
        if (e.key === "ArrowUp") {
          e.preventDefault();
          this.selectedFileIdx = Math.max(this.selectedFileIdx - 1, 0);
          this.renderFileList();
          return;
        }
        if (e.key === "Tab" || (e.key === "Enter" && this.selectedFileIdx >= 0)) {
          e.preventDefault();
          this.pickFile(this.selectedFileIdx);
          return;
        }
        if (e.key === "Escape") {
          e.preventDefault();
      ...
      if (e.key === "Backspace" && inputEl.value === "" && this.contextChips.length > 0) {
        e.preventDefault();
        this.removeChip(this.contextChips[this.contextChips.length - 1].id);
        return;
      }
      ...
      if (visible && cmds.length > 0) {
        if (e.key === "ArrowDown") {
          e.preventDefault();
          this.selectedCmdIdx = Math.min(this.selectedCmdIdx + 1, cmds.length - 1);
          this.showCmds(cmds);
          return;
        }
        if (e.key === "ArrowUp") {
          e.preventDefault();
          this.selectedCmdIdx = Math.max(this.selectedCmdIdx - 1, 0);
          this.showCmds(cmds);
          return;
        }
        if (e.key === "Tab" || (e.key === "Enter" && this.selectedCmdIdx >= 0)) {
          e.preventDefault();
          this.pickCmd(this.selectedCmdIdx);
          return;
        }
        if (e.key === "Escape") {
          e.preventDefault();
      ...
      if (e.key === "Enter" && !e.shiftKey) {
        ...
        e.preventDefault();
        if (this.isStreaming) this.vscode.postMessage({ type: "cancel" });
        this.clearInput();
      }
    });
    ...
    commandAutocomplete.addEventListener("mouseover", (e) => {
      const item = (e.target as HTMLElement).closest(".command-item");
      if (item) {
        this.selectedCmdIdx = parseInt(item.getAttribute("data-index") || "0", 10);
        this.showCmds(this.filteredCmds(inputEl.value.split(/\s/)[0]));
      }
    });
    ...
    messagesEl.addEventListener("keydown", (e) => {
      ...
      if (e.key === "ArrowDown" && idx < msgs.length - 1) {
        e.preventDefault();
        (msgs[idx + 1] as HTMLElement).focus();
    ...
    this.el.connectBtn.addEventListener("click", () => {
      this.vscode.postMessage({ type: "connect" });
    });
    modeSelector.addEventListener("change", () => {
      updateSelectLabel(modeSelector, "Mode");
      this.vscode.postMessage({ type: "selectMode", modeId: modeSelector.value });
    });
    modelSelector.addEventListener("change", () => {
      updateSelectLabel(modelSelector, "Model");
      this.vscode.postMessage({ type: "selectModel", modelId: modelSelector.value });
    });
    ...
    this.el.filePicker.addEventListener("mousedown", (e) => {
      e.preventDefault();
    });
    ...
    this.el.filePicker.addEventListener("mouseover", (e) => {
      const item = (e.target as HTMLElement).closest(".file-picker-item");
      if (item) {
        this.selectedFileIdx = parseInt(item.getAttribute("data-index") || "0", 10);
        this.renderFileList();
      }
    });
    this.el.chipStack.addEventListener("click", (e) => {
      ...
      if (btn) {
        const id = btn.closest(".context-chip")?.getAttribute("data-chip-id");
  ...
  private showPlan(entries: PlanEntry[]): void {
    ...
    this.planEl.innerHTML =
      '<div class="plan-header"><span class="plan-title">Agent Plan</span><span class="plan-progress">' +
      done +
      "/" +
      entries.length +
      "</span></div>" +
      ...
      entries
        .map(
          (e) =>
            '<div class="plan-entry plan-entry-' +
            e.status +
            " plan-priority-" +
            e.priority +
  ...
  private detectAt(value: string, cursor: number): { pos: number; query: string } | null {
    for (let i = cursor - 1; i >= 0; i--) {
      if (value[i] === "@" && (i === 0 || /\s/.test(value[i - 1])))
        return { pos: i, query: value.slice(i + 1, cursor) };
  ...
  private handleFileSearch(): void {
    ...
    if (trigger) {
      ...
      this.fileDebounce = setTimeout(() => {
        this.vscode.postMessage({ type: "fileSearch", query: trigger.query });
      }, 100);
    } else this.hideFiles();
  }
  ...
  private renderFileList(): void {
    ...
    filePicker.innerHTML = this.fileResults
      .map((r, i) => {
        const name = r.isDirectory ? `${r.fileName}/` : r.fileName;
        return (
          '<div class="file-picker-item' +
          (i === this.selectedFileIdx ? " selected" : "") +
          '" data-index="' +
          i +
          '">' +
          '<span class="file-picker-name">' +
          escapeHtml(name) +
          '</span><span class="file-picker-path">' +
          escapeHtml(r.relativePath) +
          "</span></div>"
        );
      })
      .join("");
  ...
  private pickFile(index: number): void {
    ...
    if (this.contextChips.some((c) => c.filePath === r.path)) {
      this.hideFiles();
      this.removeAtText();
      return;
    }
  ...
  private removeChip(id: string): void {
    this.contextChips = this.contextChips.filter((c) => c.id !== id);
    this.renderChips();
  }
  ...
  private renderChips(): void {
    if (this.contextChips.length === 0) {
      this.el.chipStack.innerHTML = "";
      return;
    }
    this.el.chipStack.innerHTML = this.contextChips
      .map((c) => {
        const label = c.isDirectory
          ? `${c.fileName}/`
          : c.range
            ? `${c.fileName}:${c.range.startLine}-${c.range.endLine}`
            : c.fileName;
        return (
          '<div class="context-chip" data-chip-id="' +
          escapeHtml(c.id) +
          '"><span class="chip-name">' +
          escapeHtml(label) +
          '</span><button class="chip-remove">&times;</button></div>'
        );
      })
      .join("");
  }
  ...
  private handleMessage(msg: ExtensionMessage): void {
    ...
    switch (msg.type) {
        ...
        if (msg.text || (msg.contextChipNames && msg.contextChipNames.length > 0)) {
          ...
          if (msg.contextChipNames && msg.contextChipNames.length > 0) {
            el.innerHTML =
              ...
              msg.contextChipNames.map((n) => `<span class="context-badge">${escapeHtml(n)}</span>`).join("") +
              "</div>" +
              escapeHtml(msg.text || "");
          }
          this.showThinking();
      ...
      case "streamChunk":
        ...
        if (!this.currentMsg) {
          this.hideThinking();
          this.currentMsg = this.addMessage("", "assistant");
        }
        if (msg.text) {
      ...
      case "instanceChanged": {
        ...
        if (cached) {
          this.contextChips = cached.contextChips;
          this.commands = cached.availableCommands;
          this.renderChips();
          if (cached.usageMeterText) {
            this.el.usageMeterEl.textContent = cached.usageMeterText;
            this.el.usageMeterEl.style.display = "inline";
          } else {
      ...
      case "addContextChipFromEditor":
        if (
          msg.chip &&
          !this.contextChips.some(
            (c) =>
              c.filePath === msg.chip?.filePath &&
              c.range?.startLine === msg.chip?.range?.startLine &&
              c.range?.endLine === msg.chip?.range?.endLine,
          )
        ) {
  ...
  private appendThought(text: string): void {
    ...
    if (!this.thoughtEl) {
      ...
      this.thoughtEl.innerHTML =
        '<summary class="thought-header"><span class="thought-title">Thinking...</span></summary><div class="thought-content"></div>';
  ...
  private finalizeMsg(): void {
    if (this.currentMsg && this.currentText.trim()) {
      this.currentMsg.innerHTML =
        (marked.parse(this.currentText) as string) + getToolsHtml(this.tools, this.expandedToolId);
      this.msgTexts.set(this.currentMsg, this.currentText);
    }
  }
}
